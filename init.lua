...
do
	...
	if update then
		...
		coroutine_wrap(function()
			...
			local _, lastVersion = require("coro-http").request("GET", "https://raw.githubusercontent.com/Lautenschlager-id/Fromage/master/package.lua")
			if lastVersion then
				...
				if version ~= lastVersion then
					...
					if isSemi then
						repeat
							os_log("↑info↓[UPDATE]↑ New version ↑highlight↓Fromage@" .. lastVersion .. "↑ available.")
					...
					if confirmation == 'Y' then
						for i = 1, #pkg.files do
							os_remove("deps/fromage/" .. pkg.files[i]) -- avoids permissions error
						end
						os_execute("lit install Lautenschlager-id/fromage") -- Installs the new lib
